...
#### Facebook â€” `apify/facebook-posts-scraper`
...
- Usage: Provide Facebook Page URL â†’ agent extracts last N posts with metrics

#### Instagram â€” `apify/instagram-post-scraper` + `apify/instagram-reel-scraper`
...
- Usage: Provide Instagram handle â†’ agent collects post + reel data separately

#### YouTube â€” `streamers/youtube-scraper`
...
- Usage: Provide YouTube channel URL â†’ agent extracts video performance data

#### TikTok â€” `clockworks/free-tiktok-scraper`
...
- Usage: Provide TikTok profile URL â†’ agent collects video metrics

#### Apify Integration Workflow
```
1. User provides social profile URL(s)
2. Agent selects appropriate Apify actor for platform
3. Run scraper via Apify MCP â†’ receive structured JSON data
...
- Each actor has different rate limits â€” respect platform ToS

### Telegram Bot Order Logs
When the Deliver Agent's Telegram Bot is running (SePay webhook â†’ Telegram notifications), order data accumulates in bot chat history.
...
```
Telegram Bot messages â†’ Parse order data (regex/structured)
â†’ Extract: date, amount, product, customer
â†’ Feed into Revenue Report KPI framework
â†’ Generate: daily/weekly/monthly revenue reports
```
...
**Growth Indicators**
- Follower growth rate = (New - Lost) / Total x 100
...
**Financial Health**
- Refund rate = Refunds / Total orders x 100
...
- Repeat purchase rate = Returning / Total customers x 100
...
### Trend Analysis
**Week-over-Week (WoW)**
- Calculate: (This week - Last week) / Last week x 100
- Identify momentum shifts
- Spot seasonal patterns
- React to sudden changes

**Month-over-Month (MoM)**
- Calculate: (This month - Last month) / Last month x 100
...
1. Awareness: Reach â†’ Profile visits
2. Interest: Profile visits â†’ Link clicks
3. Consideration: Link clicks â†’ Landing page views
4. Purchase: Landing page â†’ Completed orders
5. Retention: First purchase â†’ Repeat orders
...
### Trend Descriptions
"Engagement rate increased 23% WoW (1.8% â†’ 2.2%), driven primarily by Reels content which averaged 4.1% engagement vs 1.5% for static posts."
...
### Apify MCP â€” Social Data Scraping
Primary data collection tool. See "Automated Data Collection" section above for platform-specific actors:
- `apify/facebook-posts-scraper` â€” Facebook Page posts + engagement
- `apify/instagram-post-scraper` â€” Instagram feed posts
- `apify/instagram-reel-scraper` â€” Instagram Reels metrics
- `streamers/youtube-scraper` â€” YouTube channel videos
- `clockworks/free-tiktok-scraper` â€” TikTok profile videos
